...
export default function HrNotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await hrApi.get('/notifications?limit=10');
  ...
  const handleClick = async (n) => {
    try {
      await hrApi.put(`/notifications/${n.id}/read`);
  ...
  const markAllRead = async () => {
    try {
      await hrApi.put('/notifications/read-all');
      setUnreadCount(0);
  ...
  return (
    <div className="notif-bell" ref={ref} style={{ display: 'inline-block', marginLeft: 8 }}>
      <button className="notif-bell-btn" onClick={toggleOpen} title="HR Notifications">
